...
export const GET = withAuth(async (req) => {
  ...
  const { jobs, total } = listJobs({
    ...
    scoreSuccessMin: url.searchParams.get('score_success_min') ? parseInt(url.searchParams.get('score_success_min')!) : undefined,
    scoreSuccessMax: url.searchParams.get('score_success_max') ? parseInt(url.searchParams.get('score_success_max')!) : undefined,
    excludeRechecked: url.searchParams.get('exclude_rechecked') === '1' || undefined,
    createdAfter: url.searchParams.get('created_after') || undefined,
    q: q || undefined,
    sort,
    order,
    sort2: url.searchParams.get('sort2') || undefined,
    order2: url.searchParams.get('order2') || undefined,
    sort3: url.searchParams.get('sort3') || undefined,
    order3: url.searchParams.get('order3') || undefined,
    limit,
    offset,
  });
