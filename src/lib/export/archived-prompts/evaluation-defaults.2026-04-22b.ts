...
- Series-B AI startup, AI Platform TPM (verified funding, not top-tier brand) = 7/6 (software infra is candidate's strength via cloud-migration background; healthy-startup boost applies)
- Series-C AI startup, Compute Resource Programs TPM = 7/6 (compute / training-strategy work matches candidate; healthy-startup boost applies)
- Series-A AI startup, Cross-functional TPM, generic JD but verified funding = 6/5 (verified funding ≠ unverifiable; boost applies; do NOT auto-skip as "tiny unverifiable")
...
- **down_level**: Role TITLE/LEVEL is clearly below candidate (e.g., Associate, Junior, Coordinator, L4/TPM II). This is the PRIMARY signal that should reduce score_success — senior titles (Staff/Principal/Director/VP) are score_success neutral, but down_level titles DO reduce it. Do NOT use for domain mismatch — use domain_gap tags instead.
